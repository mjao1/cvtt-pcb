...
void ST7789_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
	...
	// Send pixel data with correct byte order (MSB first)
	ST7789_DC_Set();
	for (uint32_t i = 0; i < w * h; i++) {
		uint8_t pixel_data[] = {data[i] >> 8, data[i] & 0xFF};
		HAL_SPI_Transmit(&ST7789_SPI_PORT, pixel_data, sizeof(pixel_data), HAL_MAX_DELAY);
	}
